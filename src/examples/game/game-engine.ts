...
export const MAP_W = 8, MAP_H = 6, MAX_LAYER = 3, STAGE_PER_LAYER = 4, MAX_GEN = 3;
...
export interface Unit {
  ...
  kills: number; cooldowns: number[];
  generalData?: General;
}
export interface Cell { x: number; y: number; terrain: "normal"|"obstacle"; }
export type Phase = "recruit"|"player_turn"|"enemy_turn"|"battle_result"|"game_over"|"victory";
...
export class GameEngine {
  private s: GameState;
  private uid = 0;
  constructor() { this.s = this.init(); }
  private init(): GameState {
    return { phase:"recruit", layer:1, stage:1, turn:1, map:this.mkMap(),
      playerUnits:[], enemyUnits:[], generals:[], stageInfo:null,
      selectedUnitId:null, log:["⚔️ 欢迎来到将领战棋！请先招募将领。"], moveHL:[], atkHL:[] };
  }
  private mkMap(): Cell[][] {
    return Array.from({length:MAP_H},(_,y)=>Array.from({length:MAP_W},(_,x)=>({x,y,terrain:"normal" as const})));
  }
  ...
  private movable(u:Unit):{x:number;y:number}[] {
    const r:{x:number;y:number}[]=[], vis=new Set([`${u.x},${u.y}`]);
    const q=[{x:u.x,y:u.y,s:0}];
    while(q.length){const{x,y,s}=q.shift()!;if(s>0)r.push({x,y});if(s>=u.moveRange)continue;
    for(const[dx,dy]of[[0,1],[0,-1],[1,0],[-1,0]]){const nx=x+dx,ny=y+dy,k=`${nx},${ny}`;
    if(!vis.has(k)&&!this.blocked(nx,ny,u.id)){vis.add(k);q.push({x:nx,y:ny,s:s+1})}}}return r;
  }
  private atkRange(u:Unit):{x:number;y:number}[] {
    ...
    for(let dy=-mr;dy<=mr;dy++)for(let dx=-mr;dx<=mr;dx++){
      ...
      const nx=u.x+dx,ny=u.y+dy;if(nx>=0&&nx<MAP_W&&ny>=0&&ny<MAP_H)r.push({x:nx,y:ny});}return r;
  }
  private clearHL(){this.s.selectedUnitId=null;this.s.moveHL=[];this.s.atkHL=[];}
  private resetFlags(units:Unit[]){for(const u of units){u.hasMoved=false;u.hasActed=false;
  ...
  private async beginStage():Promise<GameState>{
    ...
    for(let i=0,n=3+Math.floor(Math.random()*4);i<n;i++){
      const ox=2+Math.floor(Math.random()*(MAP_W-4)),oy=1+Math.floor(Math.random()*(MAP_H-2));
      this.s.map[oy][ox].terrain="obstacle";}
    this.s.log.push(`👹 生成敌人...`);
    const enemies=await generateEnemies(layer,stage,info.terrain);
    this.s.enemyUnits=enemies.map((e,i)=>({id:this.nid(),name:e.name,emoji:e.emoji,
      hp:e.hp,maxHp:e.hp,atk:e.atk,def:e.def,spd:e.spd,moveRange:e.moveRange,skills:e.skills,
      x:MAP_W-1,y:Math.min(1+i*2,MAP_H-1),isEnemy:true,isBoss:e.isBoss,
      hasMoved:false,hasActed:false,kills:0,cooldowns:e.skills.map(()=>0)}));
    this.s.log.push(`⚔️ 敌人：${this.s.enemyUnits.map(e=>`${e.emoji}${e.name}`).join(", ")}`);
    this.s.playerUnits=this.s.generals.map((g,i)=>({id:this.nid(),name:g.name,emoji:g.emoji,
      hp:g.hp,maxHp:g.hp,atk:g.atk,def:g.def,spd:g.spd,moveRange:g.moveRange,skills:g.skills,
      x:0,y:Math.min(1+i*2,MAP_H-1),isEnemy:false,isBoss:false,
      hasMoved:false,hasActed:false,kills:0,cooldowns:g.skills.map(()=>0),generalData:g}));
  ...
  selectUnit(id:string):GameState{
    ...
    const u=this.s.playerUnits.find(u=>u.id===id);if(!u)throw new Error("无此单位");
  ...
  async attack(atkId:string,defId:string):Promise<GameState>{
    if(this.s.phase!=="player_turn")throw new Error("非玩家回合");
    const a=this.s.playerUnits.find(u=>u.id===atkId),d=this.s.enemyUnits.find(u=>u.id===defId);
    if(!a||!d)throw new Error("无此单位");if(a.hasActed)throw new Error("已行动");
    if(Math.abs(a.x-d.x)+Math.abs(a.y-d.y)>1)throw new Error("超出范围");
    const r=await calculateDamage({name:a.name,atk:a.atk,skills:a.skills},{name:d.name,def:d.def,hp:d.hp},-1);
    ...
    if(d.hp<=0){this.s.log.push(`💀 ${d.emoji}${d.name} 被击败！`);
      this.s.enemyUnits=this.s.enemyUnits.filter(u=>u.id!==defId);a.kills++;}
    this.clearHL();
    if(!this.s.enemyUnits.length)return this.onWin();
    return this.getState();
  }

  async skill(uid:string,si:number,tid:string):Promise<GameState>{
    if(this.s.phase!=="player_turn")throw new Error("非玩家回合");
    const u=this.s.playerUnits.find(u=>u.id===uid),t=this.s.enemyUnits.find(u=>u.id===tid);
    if(!u||!t)throw new Error("无此单位");if(u.hasActed)throw new Error("已行动");
    if(si<0||si>=u.skills.length)throw new Error("技能不存在");
    ...
    const r=await calculateDamage({name:u.name,atk:u.atk,skills:u.skills},{name:t.name,def:t.def,hp:t.hp},si);
    t.hp=Math.max(0,t.hp-r.damage);u.hasActed=true;u.hasMoved=true;u.cooldowns[si]=sk.cooldown;
    this.s.log.push(`🌟 ${u.emoji}${u.name} 使用「${sk.name}」！`);
    this.s.log.push(`⚔️ ${r.narrative}`);
    if(r.isCritical)this.s.log.push(`💥 暴击！`);
    ...
    for(const e of[...this.s.enemyUnits]){
      ...
      try{
        const act=await decideEnemyAction(
          {name:e.name,atk:e.atk,hp:e.hp,maxHp:e.maxHp,moveRange:e.moveRange,skills:e.skills,x:e.x,y:e.y},
          this.s.playerUnits.filter(u=>u.hp>0).map(u=>({name:u.name,hp:u.hp,x:u.x,y:u.y})),MAP_W,MAP_H);
