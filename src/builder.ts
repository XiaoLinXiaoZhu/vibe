...
export class FunctionCallBuilder<T = unknown> {
  /** 缓存执行结果的 Promise，防止 .then() 多次调用重复触发 LLM */
  ...
  withSchema<S>(schema: z.ZodType<S>): Promise<S> {
    ...
    this._promise = null; // schema 变更时清除缓存，重新执行
  ...
  private execute(): Promise<T> {
    // 缓存 Promise：同一个 builder 实例只执行一次 handleCall
    if (!this._promise) {
      this._promise = this.vibeInstance.handleCall(
        this.functionName,
        this.args,
        this.outputSchema,
        this.vibeProxy,
